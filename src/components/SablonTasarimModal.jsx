...
import { X, Save, Loader2, Check, Palette, Layout } from 'lucide-react';
...
function renderKlasik(ctx, color) {
  ...
  // Konuşmacı fotoğraf dairesi (sağ)
  ctx.beginPath();
...
function renderMinimal(ctx, color) {
  ...
  // Renk şeridi başlık altında
  ctx.fillStyle = color;
  ctx.fillRect(60, 352, 170, 8);

  // Konuşmacı foto alanı (sağ, noktalı çerçeve)
...
// ── Ana bileşen ───────────────────────────────────────────────────────────────
const SablonTasarimModal = ({ onKaydet, onClose }) => {
          ...
          <div className="w-72 flex-shrink-0 space-y-6">
            ...
            <button
              ...
              className={`w-full py-3.5 rounded-xl font-bold text-white flex items-center justify-center gap-2 transition-all shadow-md ${
                kaydedildi
                  ? 'bg-green-500'
                  : 'bg-gradient-to-r from-amare-purple to-amare-blue hover:opacity-90 disabled:opacity-60'
              }`}
            >
              {kaydediliyor
            ...
            <p className="text-xs text-gray-400 text-center leading-relaxed">
              Kaydedilen şablon 1080×1080 px PNG olarak Firestore'a yüklenir ve "Görsel Oluştur" akışında kullanılabilir.
            </p>
          </div>

          {/* Sağ panel: Canvas önizleme */}
          <div className="flex-1 flex flex-col items-center justify-start">
            <div className="text-xs text-gray-400 mb-2 font-semibold">CANLI ÖNİZLEME (1080×1080)</div>
            <div

...
export default function IlkYetmisIki({
  ...
  adimlar: string[];
  mevcut: { adim: number; planlanan_zaman: string }[];
}) {
  ...
  return (
    <div className="space-y-3">
      {adimlar.map((metin, i) => (
        <div key={i} className="rounded-2xl border border-gold/25 bg-midnight-card/60 p-4">
          <div className="flex items-start gap-3">
            ...
            <p className="text-sm leading-relaxed text-slate-200">{metin}</p>
          </div>
          ...
          <input
            ...
            className="mt-1 h-11 w-full rounded-xl border-2 border-white/15 bg-white/[0.04] px-3 text-sm text-slate-100 outline-none focus:border-gold"
          />
        </div>
      ))}

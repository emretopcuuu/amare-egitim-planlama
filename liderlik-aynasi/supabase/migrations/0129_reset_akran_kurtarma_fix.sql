-- [ACİL DÜZELTME] yeni_kamp_hazirla() reset fonksiyonu, FAZ 3'te DÜŞÜRÜLEN
-- akran_kurtarma tablosunu hâlâ siliyordu → "relation does not exist" ile
-- patlıyor, admin "Sıfırlama tamamlanamadı" görüyordu. Satır kaldırıldı.
-- Ayrıca bu oturumda eklenen soz_aksiyon_tamam tablosu (FAZ 6) ve yeni günlük-kilit
-- settings anahtarları (checkin_cipa_/ufuk_toren_kontrol_/sahit_ozeti_) temizliğe
-- eklendi. Gövde 0109 ile birebir; yalnız bu farklar.
...
begin
  ...
  delete from public.soz_aksiyon_tamam;
  ...
  delete from public.ikinci_ayna;
  delete from public.eylul_aynasi;
  delete from public.is_verisi;
  delete from public.taahhut;
  delete from public.eylul_dis;
  ...
  delete from public.settings where
    key in (
      ...
      'ikinci_ayna_acik','mezuniyet_acik','onboarding_hatirlatma_kontrol_at'
    )
    ...
    or key like 'checkin_cipa_%'
    or key like 'ufuk_toren_kontrol_%'
    or key like 'sahit_ozeti_%';
end;
$function$;
